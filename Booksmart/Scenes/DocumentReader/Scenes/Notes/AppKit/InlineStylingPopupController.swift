...
protocol InlineStylingDelegate: AnyObject {
    ...
    func inlineStyling(editPageLinkAt: NSRange, page: Int?)
}
...
class InlineStylingPopupController: NSViewController {
    ...
    @IBAction func onClickPageLink(_ sender: Any?) {
        view.window?.performClose(sender)
        delegate?.inlineStyling(editPageLinkAt: selectedRange, page: selectionPageLink)
    }
    ...
    private func setLinkToButton(color: NSColor) {
        pageLinkButton.attributedTitle = NSAttributedString(string: "Page Link", attributes: [
            .font: NSFont.systemFont(ofSize: NSFont.systemFontSize),
            .foregroundColor: color
        ])
    }
	...
	override func viewWillAppear() {
        ...
        setLinkToButton(color: selectionPageLink == nil ? NSColor.labelColor : NSColor.blue)
    }
