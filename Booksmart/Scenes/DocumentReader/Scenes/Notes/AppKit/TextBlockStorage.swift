...
/// Block Position, above or below, relative to some block
///
/// This is used in ``Block/adjacentBlockDidChange(positioned:)``
enum BlockPosition {
    case above
    case below
}
...
/// Remove the list marker from a string. (e.g. "- " or "1. ")
...
class TextBlockStorage : NSTextStorage, NSLayoutManagerDelegate, InlineStylingDelegate, LinkPopupDelegate, PageLinkPopupDelegate, BlockMenuDelegate {
    // Implementation Note:
    //
    ...
    /// Get the text attributes (size, color, font, etc.) at a particular character index
    override func attributes(at location: Int, effectiveRange range: NSRangePointer?) -> [NSAttributedString.Key : Any] {
        return storage.attributes(at: location, effectiveRange: range)
    }
    
    /// Get the NSRange from the first to last character in this text storage
    func getFullRange() -> NSRange {
        return NSRange(location: 0, length: underlyingTextStorage.string.count)
    }

    /// Print blocks for debugging purposes
...
//        }
        ...
        for block in blocks {
            ...
            var blockString = string.substring(with: block.range) as String
            blockString = blockString.description.replacingOccurrences(of: "\n", with: "[n]")
            blockString = blockString.description.replacingOccurrences(of: "\r", with: "[r]")
    ...
    private func indexOutOfBlockRange(index: Int) -> Bool {
        return index < 0 || index >= blocks.count
    }
    
    private func didChangeBlock(positioned position: BlockPosition, blockAt index: Int) {
        if indexOutOfBlockRange(index: index) {
            return
        }
        ...
        if block.wantsAdjacentBlockUpdates {
            updateBlocks()
            block.adjacentBlockDidChange(positioned: position)
        }
    }

    // MARK: Append, insert, remove, replace block(s)
    ...
    // MARK: Get blocks in various ways
    ...
    /// Get block at index. Should be used instead of accessing array directly.
    ...
    // MARK: Create blocks in various ways
...
//        index += 1
    ...
    // MARK: Get coded blocks
    
    func getCodedContents() throws -> CodedTextViewContents {
        var codedBlocks = [CodedBlock]()
        
        for block in blocks
        {
            codedBlocks.append(try CodedBlock(block))
        }
        
        return try CodedTextViewContents(blocks: codedBlocks)
    }
    ...
    /// Update blocks' index and offset properties (should run after some types of text updates, to keep these properties correct)
    ...
    /// Same as updateBlocks(), but only updates the block passed as an argument.
    ...
    // MARK: InlineStylingDelegate methods
    ...
    func inlineStyling(setBlockType blockType: BlocksInfo.Types, forBlocks blockRange: Range<Int>) {
        ...
        // Block conversion has different implementation based on this distinction
    ...
    func inlineStyling(editPageLinkAt linkRange: NSRange, page: Int?) {
        textView.displayPageLinkPopup(for: linkRange, page: page)
    }
    ...
    // MARK: PageLinkPopupDelegate methods
    
    func pageLinkPopup(createLinkAt range: NSRange, setPage page: Int) {
        let textView = self.textView!
        let replacement = "Page \(page)"
        let attrRange = NSRange(location: range.location, length: replacement.utf16.count)
        ...
        storage.addAttribute(.pageLinkTo, value: page, range: attrRange)
        ...
        // should be SET only, not delete
//        delegate?.inlineStyling(toggleStyle: .pageLink, forCharacters: selectedRange, inBlockRange: selectedBlockRange)
        inlineStyling(toggleStyle: .pageLink, forCharacters: attrRange, inBlockRange: block.index..<block.index + 1)
    }
    ...
    // MARK: LinkPopupDelegate methods

    // See LinkPopupDelegate protocol.
    ...
    // MARK: BlockMenuDelegate methods

    // See BlockMenuDelegate protocol.
