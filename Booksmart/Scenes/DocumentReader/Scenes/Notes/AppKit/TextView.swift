...
//    }
    ...
    func initialize() {
        ...
        guard let contents = containerDelegate.loadChapterContents() else { return }
...
//        saveProgress()
    ...
    func saveChapterContents(async: Bool = true) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            {
                let _ = self?.containerDelegate.saveChapterContents(contents: contents)
            }
        }
    }
    ...
    public func displayPageLinkPopup(for range: NSRange, page: Int? = nil) {
        popoverManager.displayPageLinkPopup(for: range, page: page)
    }
...
//            insertionPoint.isHidden = false
        ...
        else if event.clickCount == 1
        {
            ...
            if let page = attrs[.pageLinkTo] as? Int
            {
                containerDelegate.goToPage(page)
            }
    ...
    override func mouseMoved(with event: NSEvent) {
        ...
        if attrs.keys.contains(.pageLinkTo) || attrs.keys.contains(.link)
        {
