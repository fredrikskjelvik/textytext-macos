...
class OrderedListBlock : ListBlock {
    ...
    convenience init(from coded: Coded, owner: TextBlockStorage, offset: Int, index: Int) throws {
        let string = try NSMutableAttributedString(data: coded.data, options: [:], documentAttributes: nil)
    ...
    struct Coded : Codable {
        ...
        init(_ block: Block) throws {
            ...
            if let data = block.textStorage.rtf(from: block.textRange, documentAttributes: [:]) {
                self.data = data
            } else {
    ...
    override func createBlocks(for range: NSRange, atIndex blockIndex: Int, startingOffset blockOffset: Int) {
        ...
        blockStorage.textView.deferredSelection = NSRange(location: lastNewBlock.textRange.location, length: 0)
    }
    ...
    override func didChangeBlock(positioned: BlockPosition) {
        ...
        let selection = textView.deferredSelection ?? textView.selectedRange()

        textView.beginEditing()
        textView.deferredSelection = selection
