...
fileprivate func listStyle(forIndentationLevel indentationLevel: Int) -> StyleBuilder {
    ...
    let attributes: [NSAttributedString.Key: Any] = [
        .foregroundColor: NSColor.Grayscale.Body,
...
class ListBlock : Block {
    ...
    init(from coded: Coded, owner: TextBlockStorage, offset: Int, index: Int) throws {
        ...
        try super.init(owner: owner, type: .list, style: listStyle(forIndentationLevel: indentationLevel), data: coded.data, offset: offset, index: index)
    }
    ...
    override func didReplaceCharacters(in range: NSRange, with string: String) {
        let markerEnd = offset + markerLength

        if range.location >= markerEnd {
            return super.didReplaceCharacters(in: range, with: string)
        }
        ...
        textBlock.didReplaceCharacters(in: range, with: string)
    }
    ...
    override func willMerge(didDeleteCharacters deletedCount: Int = 0) -> Int {
        ...
        textView.deferredSelection = NSRange(location: offset, length: 0)
