...
class CodeBlock: Block {
    ...
    public init(owner: TextBlockStorage, range: NSRange, index: Int = 0, isLast: Bool? = nil) {
        ...
        if (isLast ?? (index == blockStorage.blocks.count)) {
            blockStorage.textView.deferredSelection = NSRange(location: range.location, length: 0)
    ...
    override func didDeleteLastCharacter() {
        ...
        if index == (blockStorage.blocks.count - 1) {
            blockStorage.textView.deferredSelection = NSRange(location: oldRange.upperBound - 1, length: 0)
            addTrailingTextBlock()
        }
    }

    override func processMarkdown(in inserted: String, at index: Int) {
        applyStyles()
    }
    ...
    override func drawBackground(in blockRect: NSRect, isSelected: Bool) {
        ...
        if isSelected {
            ...
            NSColor.Grayscale.InputBackground.setFill()
        }
