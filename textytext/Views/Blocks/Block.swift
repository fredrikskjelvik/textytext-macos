...
class Block {
    ...
    public init(owner: TextBlockStorage, type: BlocksInfo.Types, style: StyleBuilder, data: Data, offset: Int, index: Int) throws {
        let content = try NSAttributedString(data: data, options: [:], documentAttributes: nil)
    ...
    struct Coded : Codable {
        ...
        init(_ block: Block) throws {
            if let data = block.textStorage.rtf(from: block.range, documentAttributes: [:]) {
                self.data = data
            } else {
                throw BlockCodingError.invalidData
            }
        }
    }
    ...
    func didReplaceCharacters(inMultiBlockRange range: NSRange, with string: String) {
    ...
    func didReplaceCharacters(in range: NSRange, withLines string: String, firstNewlineAt newlineIndex: String.Index) {
        let stringLength = string.utf16.count
    ...
    func didReplaceCharacters(in range: NSRange, with string: String) {
    ...
    func didInsertCharacters(at characterIndex: Int, with string: String) {
        length += string.utf16.count
        processMarkdown(in: string, at: characterIndex)
    }
    ...
    func didEdit(_ edit: BlockEdit) {
        ...
        case .multiBlockEdit(let range, let string):
            return didReplaceCharacters(inMultiBlockRange: range, with: string)

        case .replaceWithLines(let range, let string, let firstNewline):
            return didReplaceCharacters(in: range, withLines: string, firstNewlineAt: firstNewline)

        case .replace(let range, let string):
            return didReplaceCharacters(in: range, with: string)

        case .insert(let range, let string):
            return didInsertCharacters(at: range.location, with: string)
        }
    }
    ...
    private func markdownConvert(markdownRange: NSRange, type: BlocksInfo.Types) {
        ...
        textView.deferredSelection = NSRange(location: converted.textRange.location, length: 0)
    }
    ...
    func processMarkdown(in inserted: String, at index: Int) {
    ...
    func didChangeBlock(positioned: BlockPosition) {
        return
    }
